...
export const Home = ({ token }) => {
  ...
  useEffect(() => {
    ...
    return (
      <div className="bg-gradient-to-b from-blue-500 to-purple-500 min-h-screen">
         <AnimatedCat />
        <div className="bg-gradient-to-b from-blue-200 to-blue-800 text-center my-8 p-6 rounded-lg shadow-lg max-w-md mx-auto mt-24">
          ...
          <div className="user-info-container bg-transparent p-6 rounded-md shadow-xl">
            <div className="mb-2">
              ...
              <p className="text-blue-800 mb-2">{user.length > 0 && user[0].pet_user.city} </p>
              <p className="text-blue-800 mb-2">{user.length > 0 && user[0].pet_user.bio} </p>
              {user.length && (
                <Link to={`/edit-user/${user[0].id}`} className="bg-blue-500 text-white px-3 py-1 rounded-md hover:bg-blue-700">
                  Edit Profile
                </Link>
              )}
            </div>
        ...
        <div className="flex justify-center">
        <Link to="/add-pet" className="bg-yellow-500 text-white px-3 py-1 rounded-md hover:bg-yellow-700" style={{ marginBottom: "50px" }}>Add Pet</Link>
      </div>
      </div>
